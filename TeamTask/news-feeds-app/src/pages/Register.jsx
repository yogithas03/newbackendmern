...
const Register = () => {
  ...
  const handleRegister = (e) => {
    ...
    setTimeout(() => {
      // Mock registration: directly log the user in
    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-dark-bg">
      ...
      <div className="glass-panel w-full max-w-[420px] p-12 z-10 flex flex-col gap-8 animate-fade-in mx-4">
        <div className="text-center">
          <h1 className="text-gradient text-4xl font-bold mb-2">Create Account</h1>
          <p className="text-slate-400 text-sm">Join NewsFeed to personalize your experience.</p>
        </div>

        <form onSubmit={handleRegister} className="flex flex-col gap-5">
          <div className="relative flex items-center group">
            <FiUser className="absolute left-5 text-slate-400 text-lg transition-colors group-focus-within:text-primary" />
            <input 
              type="text" 
          ...
          >
            {isLoading ? (
              ...
              <>
                Sign Up <FiArrowRight className="text-lg transition-transform group-hover:translate-x-1" />
              </>
            )}
          </button>
        </form>
        ...
        <div className="text-center text-sm text-slate-400 mt-2">
          <p>Already have an account? <Link to="/login" className="text-primary font-semibold hover:opacity-80 transition-opacity">Sign In</Link></p>
        </div>
      </div>
    </div>
  );
